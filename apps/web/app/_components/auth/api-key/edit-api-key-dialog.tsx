...
export function EditApiKeyDialog({
  ...
  const form = useAuthForm({
    ...
    onSubmit: async ({ value }) =>
      // Same `Pick<any, …>` widening as in create-api-key-dialog: better-auth
      // infers the update input as `any`, so every picked key reads as required.
      await updateApiKey.mutateAsync({
        configId: apiKey.configId,
        keyId: apiKey.id,
        name: value.name.trim()
      } as Parameters<typeof updateApiKey.mutateAsync>[0])
  })
  useEffect(() => {

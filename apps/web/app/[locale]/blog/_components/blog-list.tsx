...
async function activeUiLocale() {
  const locale = await getLocale();
  return isLocale(locale) ? locale : DEFAULT_LOCALE;
}

export async function TagRail({
  selectedTag,
  tags,
}: {
  selectedTag?: string;
  ...
  const locale = await activeUiLocale();
  const allPostsHref = `${addLocalePrefix("/blog", locale)}#all-posts`;
    ...
    <div className="flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {[null, ...tags].map((tag) => {
        ...
        return (
          <Link
            key={tag ?? "all"}
            href={tag === null ? allPostsHref : addLocalePrefix(blogTagPath(tag), locale)}
...
// Blog content is English; release/publish dates stay in the English format so
// they read consistently next to the English article body (§17.3).
...
export async function PostMeta({ post }: { post: BlogPostSummary }) {
  const t = await getTranslations("blog");
...
export async function FeaturedArticle({ post }: { post: BlogPostSummary }) {
  const t = await getTranslations("blog");
  const hasCover = Boolean(post.coverPublicUrl);
  const label = post.featured ? t("featured") : t("latest");
    ...
    >
      ...
      <div className="flex flex-1 flex-col px-2 pb-2 pt-5">
        ...
        <div className="mt-auto flex items-center justify-between gap-4 pt-7">
          <span className="text-xs text-zinc-400">{published ?? t("unscheduled")}</span>

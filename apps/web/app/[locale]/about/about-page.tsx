...
const PRINCIPLE_KEYS = ["grounded", "workspace", "artifacts"] as const;
...
export function AboutPage() {
  ...
  const featuresHref = `${addLocalePrefix("/", localePrefix)}#features`;
  const changelogHref = addLocalePrefix("/changelog", localePrefix);

  const cardItems = t.raw("hero.card.items") as string[];
  const audiences = t.raw("audiences.items") as string[];
  const direction = t.raw("direction.items") as string[];
      ...
      <main>
        ...
        <div className="mx-auto max-w-6xl px-6 pb-16">
          <Link
            ...
            href={changelogHref}
          >
            {t("buildLabel", { sha: SHORT_BUILD_SHA })}
          </Link>
        </div>
      </main>

...
async function CategorySidebar({
  ...
  const t = await getTranslations("mcp.listing");
  const locale = await getLocale();
  const uiLocale = isLocale(locale) ? locale : DEFAULT_LOCALE;
  const entries = [
    { count: total, label: t("allServersLabel"), slug: "all" },
 ...
 */
export async function McpListingView({
  categories,
  counts,
  market,
  state,
        ...
        <div className="flex flex-col gap-4 border-b border-zinc-300 pb-5 dark:border-white/10">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <p className="ml-auto text-sm text-zinc-500 dark:text-zinc-400">
              {exactCount !== null
                ? t("serverCount", { count: exactCount })
                : t("serverCountOnPage", { count: market.items.length })}
            </p>
          </div>
        ...
        {market.items.length > 0 ? (
          ...
          <div className="mt-6 rounded-xl border border-zinc-300 bg-white/54 p-10 text-center dark:border-white/10 dark:bg-white/[0.03]">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">
              {hasFilters ? t("noMatchBody") : t("syncingBody")}
            </p>
          </div>
        )}

import { isPersonalOrganization } from "../dashboard-team-selector-shared";
import type {
  BillingInterval,
  BillingLedgerEntry,
  BillingOrg,
...
export function getUsageActivityKind(entry: BillingLedgerEntry): UsageActivityKind {
...
export function formatUsageActivityDetail(kind: UsageActivityKind, detail: string) {
  return `${kind} · ${detail}`;
}

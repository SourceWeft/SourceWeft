...
export function SourceWeftBrandLockup({
  ...
  const markClassName =
    size === "nav"
      ? refined
        ...
        : "h-7 w-7 sm:h-8 sm:w-8 rounded-md transition-transform duration-300 group-hover:scale-[1.03]"
      : refined
  ...
  const wordmarkClassName =
    size === "nav"
      ? refined
        ...
        : "text-[0.95rem] sm:text-[1.05rem] font-semibold tracking-[-0.02em] text-zinc-900 dark:text-white"
      : refined
  ...
  return (
    <LocaleLink
      href="/"
      className={`group inline-flex shrink-0 items-center ${gapClassName}`}
    >
      <SourceWeftBrandMark

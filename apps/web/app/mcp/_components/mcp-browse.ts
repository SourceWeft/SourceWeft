...
export function parseMcpBrowseState(
  ...
  // `filter` is the pre-sidebar name of the category param; keep old links working.
  const rawCategory = first(params.category) ?? first(params.filter);
  const category =
    rawCategory && categories.some((entry) => entry.slug === rawCategory)
      ? rawCategory
      : "all";
  return {
    category,
...
/**
 * Builds a listing URL from the current state plus a patch. Any change other
 * than an explicit cursor resets pagination, because a cursor is only valid for
...
export function mcpBrowseHref(
  ...
  if (params.size === 0 && next.view) params.set("view", "all");

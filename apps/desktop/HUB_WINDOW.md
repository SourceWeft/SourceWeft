...
Implementation branch: `codex/desktop-hub-window`, based on `ade9f241`.
...
- Workspace resources retain their existing scopes, including workspace-wide
  Artifacts. This branch does not import the separate, uncommitted PC Files work.
...
- Hub protocol, host coordination, auxiliary React view and existing Hub suites:
  99 tests passed across 15 files.
- Web `next typegen && tsc --noEmit`: passed.
- Desktop binary Rust tests: 3 passed, including relay origin/direction checks.
- Changed Web files: ESLint with zero warnings required.

The React coordination tests mock the native transport. They are not native
window acceptance tests. Computer Use reported that the Mac was locked and
could not be automatically unlocked. Consequently **real native focus, system

...
const compressionReports = new Map<string, SourceWeftContextCompressionReport>();
...
function mergeReport(
  ...
  return {
    ...
    estimatedInputTokensBefore: Math.max(
    ...
    triggerReason: uniqueReasons(
      previous?.triggerReason,
      next.triggerReason,
    ),
    ...
    contextLength: next.contextLength ?? previous?.contextLength ?? DEFAULT_CONTEXT_LENGTH,
    usableInputTokens:
...
function assertCurrentTurnToolResultsFit(
  ...
  for (const message of currentTurnMessages) {
    if (
      !isToolResultFromTool(
        message,
        OVERSIZED_CURRENT_TURN_TOOL_NAMES,
      )
    ) {
      continue;
    }
...
export function fallbackSourceWeftMessagesToRecentWindow(
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (aiMessage.tool_calls?.some((toolCall) => toolCall.id === message.tool_call_id)) {
      continue;
    }
...
function findMatchingAIMessage(
  ...
  for (let index = previousMessages.length - 1; index >= 0; index -= 1) {
    ...
    if (
      ...
      message.tool_calls?.some((toolCall) => toolCall.id === toolMessage.tool_call_id)
    ) {
      return message;
    }
  }
...
function createSourceWeftCompressionTraceMiddleware(input: {
  ...
  return createMiddleware({
    ...
    wrapModelCall: async (request, handler) => {
      const beforeCallTokens = estimateSourceWeftMessageTokens(request.messages);
      ...
      const finalizeReport = async (extraReason?: string) => {
        ...
        const toolPruned = prunedToolCount > 0 || afterTokens < beforeCallTokens;
        const summarized =
        ...
        if (beforeCallTokens >= input.settings.budget.contextEditingTriggerTokens) {

...
test("base chat system prompt treats kb as source evidence and work as user-visible working material", () => {
  ...
  assert.match(
    CHAT_SYSTEM_PROMPT,
    /Do not convert @mentioned source filenames into \/work paths/,
  );
  assert.match(
    CHAT_SYSTEM_PROMPT,
    /Do not use \/work as the first evidence source for source-grounded factual questions/,
  );
  ...
  assert.match(
    CHAT_SYSTEM_PROMPT,
    /Do not narrate approval requests/,
  );
  assert.match(
    CHAT_SYSTEM_PROMPT,
...
test("filesystem tool descriptions are generated from enabled mounts", () => {
  ...
  assert.match(
    ...
    /\/work files are database-persisted, thread-scoped Workfiles/,
  );

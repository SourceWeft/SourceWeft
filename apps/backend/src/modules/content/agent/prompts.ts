...
- Use retrieve first when the user asks a source-grounded question that can be answered from relevant passages, such as Q&A, targeted extraction, local fact lookup, semantic lookup, or finding relevant passages across sources. Do not call ls, glob, or grep before retrieve just to discover sources or try keyword guesses; retrieve is already scoped to selected/current sources.
...
- Use grep when the user's goal is lexical or regex location, such as finding where terms appear, checking whether a pattern exists, or locating identifiers, field labels, codes, URLs, names, invoice/order numbers, quoted terms, and simple OR patterns. Grep is not a required preflight step for retrieve and does not need a prior ls unless file names/paths or a file subset matter.

...
const KB_LS_TOOL_DESCRIPTION = `Lists files in a directory. In SourceWeft, /kb is the read-only workspace knowledge view already scoped to the current turn's visible selected/current sources. ls('/kb') returns the visible source files for this turn. Use it when the task depends on source identity, file enumeration, or coverage across selected/current sources. Do not call ls before retrieve or grep just to discover scope for targeted lookup; those tools are already scoped to the same visible sources.`;
...
const KB_GLOB_TOOL_DESCRIPTION = `Finds files matching a glob pattern within the visible /kb scope. Use glob under /kb to narrow selected/current sources by filename or path pattern when the task depends on file identity or a file subset. Glob results identify paths; they are not evidence for factual claims. After narrowing paths for a source-grounded answer, gather citable evidence with read_file, grep, or retrieve.`;

const KB_GREP_TOOL_DESCRIPTION = `Searches visible /kb chunks with a case-insensitive regular expression and returns matching citable lines. Use grep for exact terms, identifiers, codes, URLs, invoice/order numbers, domains, names, amounts, dates, quoted terms, explicit search/location requests, or simple OR patterns like "invoice|order|receipt". Grep can provide evidence for matched facts, but do not use grep alone as a substitute for full-source reading when the user asks for source-wide summaries, reviews, comparisons, full-document analysis, or all key points. For targeted source-grounded Q&A or field-value extraction when exact terms are unknown, use retrieve first, then grep only if retrieve is insufficient, ambiguous, or the user explicitly asks to find/search/match occurrences. Escape regex metacharacters when literal matching is needed. Broad regex patterns without literal terms require a small visible source set or a narrowed source/chunk path.`;

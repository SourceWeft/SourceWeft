...
const KB_GREP_TOOL_DESCRIPTION = `Searches /kb candidate chunks with a case-insensitive regular expression and returns matching lines. In SourceWeft, /kb is scoped to selected/current sources when sources are selected for the turn, so grep('/kb') does not need a prior ls. Use grep only when the user's goal is lexical or regex location: finding where a provided term or pattern appears, checking whether an exact string exists, or locating occurrences of identifiers, codes, URLs, names, invoice/order numbers, quoted terms, and simple OR patterns like "invoice|order|receipt". Do not use grep first for targeted source-grounded Q&A or field-value extraction such as "what is the registered domain?", "what is the invoice number?", "when does it expire?", or "how much is it?"; use retrieve first, then grep only if retrieve is insufficient, ambiguous, or the user explicitly asks to find/search/match occurrences. Escape regex metacharacters when literal matching is needed. Broad regex patterns without literal terms require a small selected/current source set or a narrowed source/chunk path.`;

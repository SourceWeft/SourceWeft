...
function requireApiKey() {
  if (!config.pdf2markdown.apiKey) {
    throw new Error("PDF2Markdown provider is not configured: PDF2MARKDOWN_API_KEY is missing");
  }
...
async function parseError(response: Response) {
  const body = (await response.json().catch(() => null)) as Pdf2MarkdownErrorBody | null;
...
async function requestJson<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const timeout = setTimeout(() => controller.abort(), config.pdf2markdown.requestTimeoutMs);
...
export function getPdf2MarkdownTaskStatus(taskId: string) {
  return requestJson<Pdf2MarkdownTaskStatus>(`/v1/tasks/${encodeURIComponent(taskId)}/status`);
}

export function getPdf2MarkdownTaskResult(taskId: string) {
  return requestJson<Pdf2MarkdownTaskResult>(`/v1/tasks/${encodeURIComponent(taskId)}/result`);
}
...
export async function downloadPdf2MarkdownResult(url: string) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`PDF2Markdown result download failed: ${response.status} ${response.statusText}`);
    }

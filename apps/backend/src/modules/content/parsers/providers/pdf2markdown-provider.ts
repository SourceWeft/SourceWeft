...
import type { DocumentParseProvider, ProviderPendingToken, ProviderParseInput } from "./types";
import type { DocumentParseMode } from "../../types";
import {
...
function parseMode(mimeType: string) {
  return (isSupportedImageMimeType(mimeType) ? "image_ocr" : "ocr_pdf") as DocumentParseMode;
}
...
function isTerminalFailureStatus(status: string) {
  return ["failed", "error", "cancelled", "canceled"].includes(normalizeTaskStatus(status));
}
...
export const pdf2MarkdownProvider: DocumentParseProvider = {
  ...
  async resume(token: ProviderPendingToken, content: Buffer) {
    ...
    if (isTerminalFailureStatus(status.status)) {
      throw new Error(`PDF2Markdown task ${token.taskId} failed with status: ${status.status}`);
    }

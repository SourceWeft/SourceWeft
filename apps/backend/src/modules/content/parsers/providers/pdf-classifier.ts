...
function classifyFromSignals(input: {
  ...
  const bitmapCoverage = input.imageOpsByPage.map((count) => Math.min(1, count > 0 ? 1 : 0));
  const summary = summarizeNumbers(bitmapCoverage);
  const textPages = input.textCharsByPage.filter((chars) => chars >= minCharsPerPage).length;
  const textPageRatio = input.pageCount > 0 ? textPages / input.pageCount : 0;
  const imageHeavyPages = bitmapCoverage.filter((coverage) => coverage > threshold).length;
  const imagePageRatio = input.pageCount > 0 ? imageHeavyPages / input.pageCount : 0;

  if (textPageRatio >= 0.8 && summary.avg <= threshold) {
    return {
      kind: "pure_text",
      confidence: Math.min(0.99, 0.7 + textPageRatio * 0.2 + (threshold - summary.avg)),
  ...
  return {
    kind: "non_pure_text",
    confidence: Math.min(0.99, 0.65 + imagePageRatio * 0.25 + (1 - textPageRatio) * 0.1),
    pageCount: input.pageCount,
    bitmapCoverage,
    reason:
      imagePageRatio >= 0.8
...
export async function classifyPdf(content: Buffer): Promise<PdfClassification> {
  ...
  for (let pageNo = 1; pageNo <= pageCount; pageNo += 1) {
    ...
    const imageOps = operatorList.fnArray.filter((fn) => imageOperatorIds.has(fn)).length;

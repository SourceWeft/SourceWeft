...
test("chunkSourceContent filters whitespace-only chunks", async () => {
  ...
  assert.equal(
    chunks.some((chunk) => chunk.text.trim().length === 0),
    false,
  );
  assert.equal(
    chunks.every(
      (chunk) =>
        content.trim().slice(chunk.startIndex, chunk.endIndex) === chunk.text,
    ),
    true,
  );
});
...
test("chunkSourceContent uses langchain recursive defaults with configured overlap", async () => {
  ...
  for (const chunk of chunks) {
    assert.equal(
      content.trim().slice(chunk.startIndex, chunk.endIndex),
      chunk.text,
    );
  }
});
...
test("chunkSourceContent keeps short trailing sections together with default size", async () => {
  ...
  assert.equal(
    chunks.some((chunk) => chunk.text.trim() === "备注"),
    false,
  );
  assert.equal(
...
test("chunkSourceContent does not start HTML chunks inside tag attributes", async () => {
  ...
  assert.equal(
    chunks.some((chunk) =>
      /^(align|valign|class|href|title)=/.test(chunk.text.trim()),
    ),
    false,
  );

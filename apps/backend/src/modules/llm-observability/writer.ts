...
export async function recordCompletedGeneration(input: {
  traceId?: string | null;
  spanId?: string | null;
  ...
  const id = createDatabaseId();
  await safelyWrite("recordCompletedGeneration", undefined, async () => {
    await db.insert(llmGenerations).values({
      id,
      traceId: input.traceId ?? createDatabaseId(),
      ...
      rawCaptureMode: input.rawCaptureMode ?? "none",
...
export async function recordCompletedSpan(input: {
  traceId?: string | null;
  spanId: string;
  parentSpanId?: string | null;
  teamId: string;
  ...
  const id = createDatabaseId();
  await safelyWrite("recordCompletedSpan", undefined, async () => {
    await db.insert(llmSpans).values({
      id,
      traceId: input.traceId ?? createDatabaseId(),
      spanId: input.spanId,

...
<todo_visibility>
- DeepAgents todo lists are user-visible progress, not an internal scratchpad.
- Use todos only when a task benefits from visible progress tracking.
- Keep todos outcome-oriented, stage-level, and understandable without knowing tools, files, paths, providers, or implementation details.
- Do not put internal inspection, setup, tool calls, temporary file work, dependency installation, or other implementation/preparation details in todos.
- For simple tasks, answer directly without todos.
</todo_visibility>

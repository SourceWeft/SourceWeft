...
export async function* runDirectGenerateImageCommand(input: {
  ...
  const sourceweftOperationId = `sourceweft:${AGENT_TOOL_NAMES.generateImage}:${randomUUID()}`;
  ...
  const initialToolCall: ToolCallTrace = {
    ...
    latencyMs: null,
    error: null,
    sequence: input.resolveToolCallSequence(sourceweftOperationId),
  };
  if (input.traceContext) {
    await startSpan({
      ...input.traceContext,
      spanId: sourceweftOperationId,
  ...
  renderBlocks.appendGeneratedImage(sourceweftOperationId);
  yield {
  ...
  const emitDirectProgress = (
    ...
    const progressEvent = input.normalizeGeneratedImageProgressEvent({
      ...
      toolCallId: sourceweftOperationId,
      tool: AGENT_TOOL_NAMES.generateImage,
  ...
  try {
    ...
    const output = await generateImageTool.invoke(normalizedInput, {
      toolCall: { id: sourceweftOperationId },
    } as never);

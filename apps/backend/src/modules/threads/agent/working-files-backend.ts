...
function isWorkingFileNotFoundError(error: unknown) {
  return (
    error instanceof ContentError &&
    (error.statusCode === 404 || error.code === "WORKING_FILE_NOT_FOUND")
  );
}
...
export class WorkingFilesBackend implements BackendProtocolV2 {
  ...
  async ls(path = WORK_ROOT): Promise<LsResult> {
    try {
      ...
      for (const file of files) {
        const childPath = directChildPath({
          base: normalized,
          candidate: file.path,
        });
        if (!childPath) {
      ...
      return {
        files: Array.from(children.values()).sort((a, b) =>
    ...
    try {
      ...
      return {
        files: candidatePaths(files)
          .filter((candidate) =>
            matcher.test(candidate.path.replace(/\/$/, "")),
          )
  ...
  async read(
    ...
    try {
      ...
      const boundedLimit = Math.max(
        1,
        Math.min(limit, DEFAULT_READ_LINE_LIMIT),
      );

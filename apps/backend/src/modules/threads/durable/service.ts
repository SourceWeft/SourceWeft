...
async function releaseSandboxLeaseForTerminalRun(
...
function getToolConfirmationStatus(
  value: unknown,
...
function toThinkingStepTrace(value: unknown): ThinkingStepTrace | null {
  const record = getObjectRecord(value);
  if (!record) {
    return null;
  }
...
function finalizeTerminalSnapshotTrace(
  snapshot: ChatRunSnapshot,
): ChatRunSnapshot {
...
function resolveAssistantMessageProjection(input: {

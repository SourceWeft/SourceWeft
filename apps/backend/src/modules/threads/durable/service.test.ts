...
test("stale active run recovery preserves terminal assistant metadata", async () => {
  const run = createRun({
    snapshotJson: {
      assistantContent: "continued answer",
      assistantMessage: {
        id: "assistant-message-1",
        content: "Recovered snapshot answer",
        contentJson: {
          version: 1,
          parts: [{ type: "text", text: "Recovered snapshot answer" }],
        },
        metadata: {
          threadRun: {
            id: "run-1",
  ...
  let assistantUpdateContent: string | undefined;
  let assistantUpdateContentJson: Record<string, unknown> | undefined;
  ...
  await testExports.failStaleActiveRunWithDependencies(run, {
    ...
    updateAssistantMetadata: async (input) => {
      ...
      assistantSnapshot = input.snapshot as Record<string, unknown> | undefined;
      const projection = testExports.resolveAssistantMessageProjection({
        currentContent: null,
  ...
  assert.deepEqual(assistantSnapshot, snapshotJson);
  assert.equal(
    assistantUpdateContent,
    "Recovered snapshot answer\ncontinued answer",
  );
  assert.deepEqual(assistantUpdateContentJson, {
    version: 1,
    parts: [
      { type: "text", text: "Recovered snapshot answer\ncontinued answer" },
    ],
  });
});
...
test("forced stop terminalizes cancel_requested run and emits terminal events", async () => {
  ...
  assert.deepEqual(
    appended.map((event) => parseSseData(event.payload)),
    [
      {
        type: "error",

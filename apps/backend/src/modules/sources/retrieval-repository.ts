...
async function assertVectorQueryCompatible(
  ...
  if (input.embeddingIdentity.profileId !== input.embeddingProfileId)
    throw new EmbeddingIdentityError(
      "Query profile does not match the embedding identity",
    );
  await assertEmbeddingIdentityCurrent(tx, input.embeddingIdentity);
  if (input.embeddingIdentity.requestedDimensions === null) {
    // Keep the dimension check and distance query consistent with first writes.
    await tx
  ...
  const mismatch = await tx.execute(sql`
    select 1 from ${chunkEmbeddings} ce
    inner join ${chunks} c on c.id = ce.chunk_id
    inner join ${documents} d on d.id = c.document_id
    inner join ${sources} s on s.id = c.source_id
    where ce.team_id = ${input.teamId} and ce.workspace_id = ${input.workspaceId}
  ...
  if (mismatch.rows.length)
    throw new EmbeddingIdentityError(
      "Stored vectors do not match the query embedding identity or dimensions",
    );
  // Older documents without identity remain unverified; do not label them as
  // generated by the current model or silently change their read strategy.
}

export async function searchChunksByVectorExact(
  input: {
    teamId: string;
    workspaceId: string;
    embeddingProfileId: string;
  ...
  dependencies: { onIdentityChecked?: () => Promise<void> } = {},
) {
  if (input.sourceIds.length === 0) {
    return [];
  }
  ...
  return db.transaction(async (tx) => {
    await assertVectorQueryCompatible(tx, input);
    await dependencies.onIdentityChecked?.();
    const rows = await tx.execute<RetrievalSqlRow>(sql`
    select
      c.id as chunk_id,
      c.document_id,
      c.source_id,
...
export async function searchChunksByVectorAnn(
  input: {
    teamId: string;
    workspaceId: string;
    embeddingProfileId: string;
  ...
  return db.transaction(async (tx) => {
    await assertVectorQueryCompatible(tx, input);
    await dependencies.onIdentityChecked?.();
    if (input.dim !== input.queryEmbedding.length)
      throw new EmbeddingIdentityError(
        "ANN dimensions do not match the query embedding",
      );
    const rows = await tx.execute<RetrievalSqlRow>(sql`
    select
      c.id as chunk_id,

...
import {
  ...
  type SystemSkillSubmission,
} from "./service";
/**
 * Imports started by the platform itself — the skills-sync tool collecting
 * public skills — rather than by a person.
 *
 * They share the same pipeline as a person's import (
 * same validation, same in-flight dedupe, same queue, same scan and triage), so
 * there is one set of rules. System tasks have no tenant ownership, grants, or installation.
 */

export { SYSTEM_SUBMITTER_ID } from "./scope";
export type SystemSubmitResult =
  | {
      source: string;
      ok: true;
      created: boolean;
      submission: SystemSkillSubmission;
    }
...
export async function submitSkillSourcesAsSystem(
  ...
  for (const item of sources) {
    ...
    try {
      const { submission, created } = await createSystemSkillSubmission({
        source,

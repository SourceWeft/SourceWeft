...
import { skillDatabaseEnabled } from "../../../test/skill-database";

describe.skipIf(!skillDatabaseEnabled)("AI analysis atomic publication", () => {
  ...
  test("all three independent locales and categories publish together", async () => {
    ...
    expect(
      (await rows(f.skillVersionId)).find((x) => x.locale === "zh-TW")!.overview
        .summary,
    ).toBe("製作圖表，整理資料。");
    ...
    expect(await repo.requestSkillAnalysis(f.skillVersionId, false)).toBeNull();

...
describe("langchain-client.transport", () => {
  ...
  afterEach(async () => {
    ...
    await Promise.all(
      protocolServers.splice(0).map((server) => server.close()),
    );
    await Promise.all(
  ...
  function protocolServer() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => ({
      content: [
        { type: "text", text: String(request.params.arguments?.value) },
      ],
  ...
  for (const mode of ["http", "sse", "compat"] as const) {
    test(`real ${mode} discovery and tool calls preserve names, headers and content`, async () => {
      ...
      assert.strictEqual(
        tools,
        same,
        "one connection for concurrent discovery",
      );
      assert.equal(tools.length, 1);
      assert.equal(
        tools[0]?.name,
        `mcp__${langChainMcpServerKey(record)}__echo`,
      );
  ...
  test("pure OAuth SSE without custom headers sends the user's token on GET and POST", async () => {
    ...
    const client = createLangChainMcpClient({
      install: {
        ...install(`${fixture.origin}/sse`, "sse"),
        authType: "oauth",
      },

(function () {
  ...
  window.HPX["typewriter-multi"] = function (el) {
    ...
    wrap.style.cssText =
      "position:absolute;inset:0;display:flex;flex-direction:column;justify-content:center;gap:14px;padding:32px 48px;font:600 24px ui-monospace,Menlo,monospace;color:var(--text-1,#e7e7ef);";
    wrap.style.fontFamily = U.font(el, "mono");
    el.appendChild(wrap);
    const rows = lines.map((txt) => {
      ...
      cur.textContent = "\u2588";
      cur.style.cssText =
        "display:inline-block;margin-left:2px;color:var(--accent,#22d3ee);animation:hpxBlink 1s steps(2) infinite;";

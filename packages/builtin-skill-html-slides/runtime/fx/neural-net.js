(function () {
  ...
  window.HPX["neural-net"] = function (el) {
    ...
    const k = U.canvas(el),
      ctx = k.ctx;
    const ac = U.accent(el, "#7c5cff"),
      ac2 = U.accent2(el, "#22d3ee");
    ...
    const layout = () => {
      ...
      for (let L = 0; L < layers.length - 1; L++) {
        const a = nodes.filter((n) => n.L === L),
          b = nodes.filter((n) => n.L === L + 1);
        for (const x of a)
          for (const y of b) edges.push([nodes.indexOf(x), nodes.indexOf(y)]);
      }
    };
    layout();
    let lw = k.w,
    ...
    const stop = U.loop((t) => {
      ...
      if (t - last > 0.25) {
        ...
        const s = starts[(U.random() * starts.length) | 0];
      ...
      for (const p of pulses) {
        ...
        if (p.t >= 1) {
          ...
          p.node2 = next[(U.random() * next.length) | 0];
          if (!p._started) {
            p._started = true;
          }
        }
      ...
      for (const p of pulses) {
        if (!p.target) {
          const next = nodes.filter((n) => n.L === p.L + 1);
          p.target = next[(U.random() * next.length) | 0];
        }
        p.t += 0.04;
        const a = p.node,
          b = p.target;

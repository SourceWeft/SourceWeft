(function () {
  ...
  window.HPX["data-stream"] = function (el) {
    ...
    const k = U.canvas(el),
      ctx = k.ctx;
    const ac = U.accent(el, "#22d3ee"),
      ac2 = U.accent2(el, "#7c5cff");
    const rows = [];
    const rh = 22;
    const genRow = (y) => ({
      y,
      dir: U.random() < 0.5 ? -1 : 1,
      speed: U.rand(30, 90),
      offset: U.random() * 2000,
      text: Array.from({ length: 120 }, () => {
        const r = U.random();
        if (r < 0.3) return U.random() < 0.5 ? "0" : "1";
        if (r < 0.6)
          return (
            "0x" +
            Math.floor(U.random() * 256)
              .toString(16)
              .padStart(2, "0")
          );
        return U.random().toString(16).slice(2, 6);
      }).join(" "),
    });
    const init = () => {
      rows.length = 0;

...
import { createNotionConnectorAdapter } from "../src/adapter";
import type { NotionAdapterRuntimeConfig } from "../src/adapter";

function testConfig(overrides: Partial<NotionAdapterRuntimeConfig> = {}): NotionAdapterRuntimeConfig {
  return {
...
test("notion adapter declares install-integration OAuth manifest", () => {
  const adapter = createNotionConnectorAdapter(testConfig());
...
test("notion adapter manifest does not require OAuth env at startup", () => {
  const adapter = createNotionConnectorAdapter(testConfig({ clientId: "" }));
  const manifest = adapter.getManifest();
  assert.deepEqual(manifest.auth.authorizationParams, {
    client_id: "",
    owner: "user",
...
test("notion client always sends API version 2026-03-11", async () => {
  ...
  await withMockedFetch(
    ...
    async (calls) => {
      for await (const _item of adapter.discover(baseDiscoverInput)) {
...
test("notion adapter declares expanded actions", () => {
  ...
  const actionTypes = new Set(
    adapter.getManifest().actions.map((action) => action.type),
  );
...
test("notion readiness checks only page access with page_size 1", async () => {
  const adapter = createNotionConnectorAdapter(testConfig());
  assert.ok(adapter.checkSyncReadiness);

  await withMockedFetch(
    () => jsonResponse({ results: [], has_more: false }),
    async (calls) => {
      const result =
        await adapter.checkSyncReadiness?.(baseDiscoverInput);
...
test("notion discover indexes pages only and never sends database filter", async () => {
  ...
  await withMockedFetch(
    ...
    async (calls) => {
      const items = [];
      for await (const item of adapter.discover(baseDiscoverInput)) {
        items.push(item);
      }
...
test("notion webhook parser maps page update to targeted sync", async () => {
  const adapter = createNotionConnectorAdapter(testConfig());
  assert.ok(adapter.parseWebhookEvent);
  assert.ok(adapter.mapWebhookEventToSyncTargets);
  ...
  const targets = await adapter.mapWebhookEventToSyncTargets(event);
...
test("notion webhook verification rejects bad signature when secret is configured", async () => {
  const adapter = createNotionConnectorAdapter(testConfig({ webhookSecret: "secret" }));
  assert.ok(adapter.verifyWebhook);
  const verifyWebhook = adapter.verifyWebhook;
...
test("notion webhook data source changes rediscover pages but database changes are record-only", async () => {
  ...
  const dataSourceEvent = await adapter.parseWebhookEvent({
    headers: {},
    query: {},
    rawBody: JSON.stringify({
      id: "evt_ds",
  ...
  assert.deepEqual(
    await adapter.mapWebhookEventToSyncTargets(dataSourceEvent),
    [
      {
        action: "sync",
        objectId: "ds_1",
  ...
  const databaseEvent = await adapter.parseWebhookEvent({
    headers: {},
    query: {},
    rawBody: JSON.stringify({
      id: "evt_db",
  ...
  assert.deepEqual(
    await adapter.mapWebhookEventToSyncTargets(databaseEvent),

...
- User @mentions, attachment labels, and source filenames refer to Source Library entries under /kb unless the user explicitly says they are Files. Do not convert @mentioned source filenames into /files paths.
...
- Do not use /files as the first evidence source for source-grounded factual questions. Use /files as persisted thread working memory only after evidence needs are clear, or when continuing generated working material.
- Create /files Files when the process itself has follow-up value: complex plans, audits, evaluations, source extraction tables, calculations, long drafts, multi-artifact preparation, or work that should be resumed later. Do not create Files just to answer a simple question, make a small edit, or produce a short final response.
- If /files contains factual claims that will appear in a source-grounded final answer, verify them against /kb or another citable source before using them.

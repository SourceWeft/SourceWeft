...
type OrganizationMembership = { role: string };

export function createBillingHttpRoutes(
  billingService: BillingService,
  host: BillingHttpHost,
) {
  const {
    workspaceService,
    onboardingService,
  ...
  return function registerBillingRoutes(app: Hono) {
    ...
    app.post("/v1/teams/:teamId/billing/subscription/checkout", async (c) => {
      ...
      const body = ensureObjectBody(await c.req.json().catch(() => null));
      const parsed =
        createTeamSubscriptionCheckoutRequestSchema.safeParse(body);
      if (!parsed.success) {
        throw ApiError.validation(
          parsed.error.flatten() as Record<string, unknown>,
        );

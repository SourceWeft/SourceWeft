...
export function createBillingAuthPlugins(input: {
  ...
  // Auth owns verification only; subscription commands use the common billing API.
  if (!plugin.endpoints.creemWebhook) return [];
  return [
    { ...plugin, endpoints: { creemWebhook: plugin.endpoints.creemWebhook } },
  ];
}

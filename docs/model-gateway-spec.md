# Internal Model Gateway Specification (Frozen)

## 1) Purpose

Define the internal TypeScript package used by app services to call the model gateway.
This package is mandatory for all model interactions.
...
Billing/budget enforcement stays in calling domain services; gateway middleware must remain side-effect safe and independently testable.
...
- enforce allowed base URL list for model gateway endpoint
...
- chat complete/stream via model gateway
...
- Provider swap requires only gateway routing/config change, no app code change

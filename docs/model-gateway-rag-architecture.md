# Model-Gateway TS Architecture (Design Freeze)
...
- `docs/model-gateway-rag-overview.md`
- `docs/model-gateway-spec.md`
...
with a pure TypeScript stack, self-hosted data plane, and a model gateway from Day 1.
...
- Design only
- Core implementation now exists in `packages/model-gateway`
...
4. LLM gateway: `Model Gateway` from Day 1 (no direct provider calls in app code)
...
10. Build a custom internal TS package: `packages/model-gateway`
...
SourceWeft/
  ...
  packages/
    ...
    model-gateway/                 # internal TS package for model access
  ...
  docs/
    architecture.md
    model-gateway-rag-architecture.md  # this document
```
...
Client/Web
  -> apps/api (Hono)
     ...
     -> packages/model-gateway -> model gateway service
...
- All model access goes through `packages/model-gateway`.
...
6. Embedding via model gateway (`embed-default`)
...
4. RRF fusion
5. Rerank via model gateway (`rerank-default`)
6. Context assembly with token budget
7. Generation via model gateway (`chat-default`)
...
Provider swaps must be done in deployment gateway configuration only.

---

## 9) Custom TS Package (`packages/model-gateway`)
...
- Use centralized secret management for gateway/provider credentials
...
3. Bootstrap `packages/model-gateway` interfaces and tests first
4. Wire API -> graph -> model-gateway minimal happy path

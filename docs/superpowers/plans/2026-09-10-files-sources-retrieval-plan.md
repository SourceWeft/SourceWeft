...
最新验收结论（2026-09-11）：Docker 经明确授权强制停止并重新启动后，原 PostgreSQL/Redis 容器健康，测试会话恢复。最后的本地引用桌面复验通过：原文件显示 `Cited version verified` 和正确原文；外部修改后显示 `File changed since cited`，默认阻止将新版内容当作旧引用，明确点击 `Open current version` 后才展示新内容。测试后已恢复原 fixture。

本轮已通过 Cloud 真实 API 与默认模型文件读取、Local 桌面目录/文本图片预览/文内搜索/真实 Agent 检索与引用、超过 1 MiB 原生下载 hash 校验及引用版本变化流程。下面的中途锁屏、数据库断连和引用预览阻塞记录现已解除。尚未验收视觉成功路径（默认模型无图像输入能力）；持续缓存和 Local Sources 等未实现项保持未完成，不能将本轮结果称为完整 Release A/B 交付。

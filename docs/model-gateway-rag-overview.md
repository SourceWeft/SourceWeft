# Model-Gateway Architecture Overview (Frozen)
...
Worker
  -> fetch from S3
  -> parse via Docling
  -> chunk + embedding via model gateway
...
- All model calls go through `packages/model-gateway`.
...
SourceWeft/
  ...
  docs/
    architecture.md
    model-gateway-rag-overview.md
    model-gateway-spec.md
```
...
- Centralized secret management for gateway/provider keys
...
1. Base platform: Hono, model gateway, Postgres, Redis, S3 wiring
